        ...
        public override PwEntry CreatePwEntry(PwDatabase pwStorage)
        {
            ...
            if (!String.IsNullOrEmpty(secureContents.reg_name))
            {
                ...
                if (!String.IsNullOrEmpty(secureContents.reg_email))
                    entry.Strings.Set("Registrierungs-E-Mail", new ProtectedString(false, StringExt.GetValueOrEmpty(secureContents.reg_email)));
            }
            else
            ...
            if (!String.IsNullOrEmpty(secureContents.download_link))
            {
                ...
                if (!String.IsNullOrEmpty(secureContents.publisher_website))
                    entry.Strings.Set("Hersteller-Webseite", new ProtectedString(false, StringExt.GetValueOrEmpty(secureContents.publisher_website)));
            }
            ...
            entry.Strings.Set("Hersteller", new ProtectedString(false, StringExt.GetValueOrEmpty(secureContents.publisher_name)));
            entry.Strings.Set("Version", new ProtectedString(false, StringExt.GetValueOrEmpty(secureContents.product_version)));
            entry.Strings.Set("Kundenservice", new ProtectedString(false, StringExt.GetValueOrEmpty(secureContents.support_email)));
            entry.Strings.Set("Bestellnummer", new ProtectedString(false, StringExt.GetValueOrEmpty(secureContents.order_number)));
